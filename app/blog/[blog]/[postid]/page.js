...
// Below correct code for dynamic seo
...
//   try {
//     blogMeta = JSON.parse(
//       decodeURIComponent(cookieStore.get("blogMeta")?.value || "{}")
//     );
//   } catch {
//     blogMeta = null;
//   }

//   // console.log(blogMeta, "blogMeta", typeof blogMeta);
...
//   const rawTitle = blogMeta?.title?.rendered || "Costa Rican Insurance - Blog";
...
import { cookies } from "next/headers";
import { getpostdetails } from "../../../lib/text"; // optional: keep inline
import BlogClient from "../BlogClient";
function decodePostId(postId) {
  if (!postId) return null;
  if (/^\d+$/.test(postId)) {
...
export async function generateMetadata() {
  ...
  if (!blogMeta?.title || !blogMeta?.postid) {
    return defaultMetadata;
  }

  // ✅ Clean up description (remove \n, trim, slice 160)
  const cleanDescription = blogMeta.description
    ?.replace(/\s+/g, " ")
    .trim()
    .slice(0, 160);

  // Canonical URL
  const url = `https://costaseo.vercel.app/blog/${blogMeta.slug}/${blogMeta.postid}`;

  // Featured image fallback
  const featuredImage =
    blogMeta.image || "https://costaseo.vercel.app/default-blog-image.jpg";

  return {
    title: {
      absolute: blogMeta.title,
    },
    description: cleanDescription,

    keywords:
      `${blogMeta.title}, Costa Rica, insurance, blog, mortgage, property`.slice(
        0,
        255
      ),
    authors: [{ name: "Costa Rican Insurance" }],
